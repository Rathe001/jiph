...
modAdGroups.directive('adGroup', [function () {
    return {
        restrict: 'EA',
        scope: {},
        bindToController: {
...
modAdSets.directive('budgetSchedule', ['Currency', 'OptimizationGoals', 'BillingEvents', function (Currency, OptimizationGoals, BillingEvents) {
    return {
        restrict: 'EA',
        scope: {},
        bindToController: {
    ...
    function controller($scope) {
        ...
        // Autobid change
        $scope.$watch(function () {
            return vm.budgetSchedule.is_autobid;
        }, function (newVal) {
        ...
        $scope.$watch(function () {
            return vm.budgetSchedule.optimization_goal;
        }, function (newVal) {
            if (newVal) {
                _setBillingEvents();
            }
...
angular.module('modAdSets').factory('BillingEvents', [function () {
    var service = {};

    // Do not change position in array
    var events = [{ id: "IMPRESSIONS", name: "Impressions" }, // 0
    ...
    function getAllValid(goal) {
        ...
        switch (goal) {
            case "APP_INSTALLS":
                output.push(events[1]);break;
            case "LINK_CLICKS":
                output.push(events[2]);break;
            case "OFFER_CLAIMS":
                output.push(events[3]);break;
            case "PAGE_LIKES":
                output.push(events[4]);break;
            case "POST_ENGAGEMENT":
                output.push(events[5]);break;
            case "VIDEO_VIEWS":
                output.push(events[6]);break;
        }
...
angular.module('modAdSets').factory('OptimizationGoals', [function () {
    var service = {};

    // Do not change position in array
    var goals = [{ id: "APP_INSTALLS", name: "App installs" }, // 0
...
modCampaigns.controller('ctrlCampaignsCreate', ['$scope', 'Accounts', 'Campaigns', 'Dictionary', 'Objectives', 'Facebook', 'Audiences', function ($scope, Accounts, Campaigns, Dictionary, Objectives, Facebook, Audiences) {
    ...
    function _addAudience(id) {
        ...
        // Get reach
        var targetingSpec = _generateReachEstimatePayload(add.targeting);

        Facebook.get('/' + Accounts.active + '/reachestimate', targetingSpec).then(function (response) {
    ...
    function _getReachEstimate(audience) {
        var targetingSpec = _generateReachEstimatePayload(audience.targeting);
    ...
    function _generateReachEstimatePayload(input) {
        ...
        for (var _i in output.targeting_spec) {
            switch (_i) {
                case "genders":
                    output.targeting_spec[_i] = [output.targeting_spec[_i]];
                    break;
                case "geo_locations":
                case "excluded_geo_locations":
                    for (var j in output.targeting_spec[_i]) {
                        if (j === "location_types") {
                            output.targeting_spec[_i][j] = [output.targeting_spec[_i][j]];
                        }
                    }
                    break;
                case "connections":
                ...
                case "friends_of_connections":
                    if (output.targeting_spec[_i]) {
                        var obj = vm.campaign.promoted_object[Object.keys(vm.campaign.promoted_object)[0]];
                        console.log(obj);
                        output.targeting_spec[_i] = [obj];
                    } else {
                        output.targeting_spec[_i] = undefined;
                    }
            }
        }
